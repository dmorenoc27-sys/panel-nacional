...
(function () {
  ...
  let E = null, META = {}, LAKE = {}, tabE = 'res', filtroMeta = '', q = '', modoInv = true, abierto = null;
  ...
  function cuerpo() {
    const b = $('ent-body'), P_ = E.presupuesto, I = E.ingresos, T = modoInv ? P_.inversiones : P_.total;
    if (tabE === 'res') {
      ...
      const A = E.alertas, al = [['dev_sin_girar', 'Proveedores sin cobrar', 'devengado sin girar > 15 días', 'bad'], ['comp_sin_devengar', 'Contratos sin avanzar', 'comprometido sin devengar > 45 días', 'warn'], ['cert_sin_comp', 'Presupuesto reservado sin usar', 'certificaciones > 60 días sin compromiso', 'p']];
      b.innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:8px">
        <div><div class="pd-lbl">¿Cuánto entra y cuánto sale?<small>ingresos vs gastos ${ANIO}, todo el presupuesto</small></div>
          <table class="pd-tbl"><tr><th></th><th>Presupuesto (PIM)</th><th>Ejecutado</th><th>Avance</th></tr><tr><td>Ingresos recaudados</td><td>${F(I.total.pim)}</td><td>${F(I.total.recaudado)}</td><td>${P(100 * I.total.recaudado / (I.total.pim || 1))}</td></tr><tr><td>Gastos devengados</td><td>${F(P_.total.pim)}</td><td>${F(P_.total.dev)}</td><td>${P(100 * P_.total.dev / (P_.total.pim || 1))}</td></tr><tr><td>Gastos pagados</td><td></td><td>${F(P_.total.pag)}</td><td>${P(100 * P_.total.pag / (P_.total.pim || 1))}</td></tr><tr><td><b>Inversiones devengadas</b></td><td>${F(P_.inversiones.pim)}</td><td>${F(P_.inversiones.dev)}</td><td>${P(100 * P_.inversiones.dev / (P_.inversiones.pim || 1))}</td></tr></table>
          <div class="pd-lbl" style="margin-top:14px">La cadena del gasto<small>${modoInv ? 'solo inversiones' : 'todo el gasto'}</small></div>
          <table class="pd-tbl">${cad.map(([k, v, d, c]) => `<tr><td style="width:130px"><b>${k}</b></td><td style="width:140px">${barra(v, T.pim, c)}</td><td style="width:110px">${F(v)}</td><td class="mutx" style="text-align:left;font-size:10.5px">${d}</td></tr>`).join('')}</table>
          <div class="pd-lbl" style="margin-top:14px">Alertas que requieren decisión</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">${al.map(([k, t, d, c]) => `<div class="al ${c}" data-a="${k}" style="padding:8px 10px"><div class="n" style="font-size:20px">${A[k].length}</div><div class="t"><b>${t}</b><br>${d}</div></div>`).join('')}</div></div>
        <div><div class="pd-lbl">¿De dónde sale el dinero?<small>por fuente · PIM, devengado y recaudado</small></div>
          <table class="pd-tbl"><tr><th>Fuente</th><th>PIM gasto</th><th>Devengado</th><th>Avance</th><th>Recaudado</th></tr>${fts.map(f => { const ing = Object.values(I.fuentes).find(x => x.nombre === f.nombre); return `<tr><td style="white-space:normal">${f.nombre}</td><td>${F(f.pim)}</td><td>${F(f.dev)}</td><td>${P(100 * f.dev / (f.pim || 1))}</td><td>${ing ? F(ing.recaudado) : '—'}</td></tr>`; }).join('')}</table>
          <div class="pd-lbl" style="margin-top:14px">Las 10 inversiones más grandes<small>PIM SIAF · devengado · avance</small></div>
          <table class="pd-tbl"><tr><th>Inversión</th><th>PIM</th><th>Devengado</th><th>Avance</th></tr>${top.map(m => `<tr class="l" data-sf="${m.sec_func}"><td style="white-space:normal"><b>${m.act_proy}</b> ${nombreMeta(m).slice(0, 80)}</td><td>${F(m.pim)}</td><td>${F(m.dev)}</td><td>${P(100 * m.dev / (m.pim || 1))}</td></tr>`).join('')}</table></div></div>`;
      b.querySelectorAll('.al').forEach(a => a.onclick = () => { tabE = 'al'; render(); }); b.querySelectorAll('tr.l').forEach(tr => tr.onclick = () => { filtroMeta = tr.dataset.sf; tabE = 'exp'; render(); });
    } else if (tabE === 'ppto') {
      const fila = (n, d) => `<tr><td style="white-space:normal">${n}</td><td>${F(d.pia)}</td><td>${F(d.ampl + d.modif)}</td><td><b>${F(d.pim)}</b></td><td>${F(d.cert)}</td><td>${F(d.comp_anual)}</td><td>${F(d.dev)}</td><td>${F(d.gir)}</td><td>${F(d.pag)}</td><td>${P(100 * d.dev / (d.pim || 1))}</td></tr>`;
      ...
      const mods = E.modificaciones.slice().reverse();
      b.innerHTML = sec('Marco presupuestal por fuente de financiamiento', 'PIM = PIA + créditos suplementarios/transferencias + modificaciones') + `<table class="pd-tbl">${head}${Object.values(P_.fuentes).filter(f => f.pim).sort((a, c) => c.pim - a.pim).map(f => fila(f.nombre, f)).join('')}${fila('<b>Total</b>', P_.total)}</table>`
        + sec('Por genérica de gasto', 'en qué tipo de gasto se usa el presupuesto') + `<table class="pd-tbl">${head}${Object.values(P_.genericas).filter(f => f.pim).sort((a, c) => c.pim - a.pim).map(f => fila(f.nombre, f)).join('')}</table>`
        + sec('Programación de compromisos anual (PCA)', 'techo de compromisos autorizado por el MEF por fuente y genérica') + `<table class="pd-tbl"><tr><th>Fuente</th><th>Genérica</th><th>PCA asignada</th><th>Certificado</th><th>Comprometido</th><th>Libre</th></tr>${E.pca.filter(p => p.asignado).sort((a, c) => c.asignado - a.asignado).map(p => `<tr><td style="white-space:normal">${p.fuente_n}</td><td style="white-space:normal">${p.generica_n}</td><td>${F(p.asignado)}</td><td>${F(p.certificado)}</td><td>${F(p.comprometido)}</td><td>${F(p.asignado - p.certificado)}</td></tr>`).join('')}</table>`
        + sec('Modificaciones presupuestales', `${mods.length} notas · la más reciente primero`) + `<table class="pd-tbl"><tr><th>Nota</th><th>Fecha</th><th>Tipo</th><th>Documento</th><th>Crédito (a)</th><th>Anulación (de)</th><th>Metas</th></tr>${mods.slice(0, 150).map(m => `<tr><td>${+m.nota}</td><td>${m.fecha || ''}</td><td style="white-space:normal">${m.tipo_n}</td><td style="white-space:normal;font-size:10.5px">${m.doc || ''}${m.notas && m.notas !== 'CREDITOS Y ANULACIONES' ? '<br>' + m.notas : ''}</td><td>${F(m.credito)}</td><td>${F(m.anulacion)}</td><td style="font-size:10.5px;white-space:normal">${m.metas.map(s => META[s] ? (META[s].es_inv ? META[s].act_proy : 'meta ' + META[s].meta) : s).join(', ')}</td></tr>`).join('')}</table>`;
    } else if (tabE === 'ing') {
      ...
      b.innerHTML = sec('Ingresos por fuente de financiamiento', 'presupuesto de ingresos (PIM) frente a lo recaudado') + `<table class="pd-tbl"><tr><th>Fuente</th><th>PIA</th><th>PIM</th><th>Recaudado</th><th>Avance</th>${meses.map(m => `<th>${MES[+m - 1]}</th>`).join('')}</tr>${fts.map(f => `<tr><td style="white-space:normal">${f.nombre}</td><td>${F(f.pia)}</td><td>${F(f.pim)}</td><td><b>${F(f.recaudado)}</b></td><td>${f.pim ? P(100 * f.recaudado / f.pim) : '—'}</td>${meses.map(m => `<td style="font-size:10px">${f.mensual[m] ? M(f.mensual[m]) : ''}</td>`).join('')}</tr>`).join('')}<tr><td><b>Total</b></td><td>${F(I.total.pia)}</td><td>${F(I.total.pim)}</td><td><b>${F(I.total.recaudado)}</b></td><td>${P(100 * I.total.recaudado / (I.total.pim || 1))}</td><td colspan="12" class="mutx" style="text-align:left;font-size:10px">mensual en S/ millones</td></tr></table>`
        + sec('Por rubro de ingreso', 'clasificador de ingresos') + `<table class="pd-tbl"><tr><th>Clasificador</th><th>Concepto</th><th>PIM</th><th>Recaudado</th><th>Avance</th></tr>${I.clasificadores.filter(c => c.pim || c.recaudado).slice(0, 80).map(c => `<tr><td>${c.cod}</td><td style="white-space:normal">${c.nombre}</td><td>${F(c.pim)}</td><td>${F(c.recaudado)}</td><td>${c.pim ? P(100 * c.recaudado / c.pim) : '—'}</td></tr>`).join('')}</table>`;
    } else if (tabE === 'pi') {
      Incentivos.render(b, E, META, E._ue.cod, exp => { q = String(+exp); filtroMeta = ''; tabE = 'exp'; render(); });
    } else if (tabE === 'inv') {
      const rows = metasSel().filter(m => hit(nombreMeta(m) + ' ' + m.act_proy + ' ' + m.sec_func)).sort((a, c) => (c.pim || c.comp) - (a.pim || a.comp));
      b.innerHTML = `<table><tr><th>${modoInv ? 'CUI · inversión' : 'Meta'}</th><th>PIM SIAF</th><th>PIM TE</th><th>Certificado</th><th>Comprometido</th><th>Devengado</th><th>Girado</th><th>Pagado</th><th>Avance</th><th>Exp.</th></tr>${rows.map(m => { const c = LAKE[m.act_proy] || {}; const av = m.pim ? 100 * m.dev / m.pim : null; return `<tr class="l" data-sf="${m.sec_func}"><td><b>${m.act_proy}</b> ${nombreMeta(m)}<br><small class="mutx">sec. func. ${m.sec_func} · meta ${m.meta}${c.cui ? ` · <a class="fc" href="#/cui/${c.cui}">ficha ↗</a>` : ''}</small></td><td>${M(m.pim)}</td><td class="mutx">${c.pim ? M(c.pim) : '—'}</td><td>${M(m.cert)}</td><td>${M(m.comp)}</td><td>${M(m.dev)}</td><td>${M(m.gir)}</td><td>${M(m.pag)}</td><td>${av != null ? P(av) : '—'}</td><td>${m.n_exp}</td></tr>`; }).join('')}</table><p class="mutx" style="font-size:11px;padding:6px 8px">S/ millones · PIM SIAF al ${E.corte}; PIM TE = Transparencia al ${R.corte}. Clic en una fila para ver sus expedientes.</p>`;
